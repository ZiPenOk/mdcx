...
async def _crawl_caribbeancom(ctx: Context, client, movie_id: str) -> CrawlerData:
    ...
    if html_content is None:
        raise CralwerException(f"official uncensored request failed: {error}")

    html = _selector(html_content)
    title = _first_xpath_text(html, '//*[@itemprop="name"]/text()', "//h1/text()")
    if not title:
        raise CralwerException("official uncensored data failed: title")
...
async def _crawl_json_site(ctx: Context, client, site: UncensoredOfficialSite, movie_id: str) -> CrawlerData:
    ...
    try:
        ...
        raise CralwerException(f"official uncensored json failed: {exc}") from exc

    if not isinstance(data, dict):
        raise CralwerException("official uncensored json failed: not object")
